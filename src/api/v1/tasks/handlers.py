...
async def create_task(
    ...
    task = await ORMTaskService().create_task(task_in=task_in.to_entity())
...
async def update_task(
    ...
    try:
        task = await ORMTaskService().update_task(task_id=task_id, task_in=task_in.to_entity())
...
async def delete_task(
    ...
    try:
        await ORMTaskService().delete_task(task_id=task_id)
    except TaskNotFound as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail={"message": e.message})
...
async def get_task(
    ...
    try:
        task: TaskEntity = await ORMTaskService().get_task_by_id(task_id=task_id)
    except TaskNotFound as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail={"message": e.message})
    return TaskSchema.from_entity(task)
...
async def get_task_list(
    ...
    tasks = await ORMTaskService().get_task_list(pagination_in)
